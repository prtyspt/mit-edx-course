...
def getAvailableLetters(lettersGuessed):
    ...
    for l in string.ascii_lowercase:
        if l not in lettersGuessed:
            availableLetters = availableLetters + l
...
def hangman(secretWord):
    ...
    * After each round, you should also display to the user the 
    ...
    while numberOfGuessesLeft > 0:
        ...
        print(getAvailableLetters(guessedLetters))
        guessedLetter = input('Please guess a letter:')
        lowercaseGuessedLetter = guessedLetter.lower()
        guessedLetters.append(lowercaseGuessedLetter)
        
        if lowercaseGuessedLetter in secretWord:
            print('Good Guess:', getGuessedWord(secretWord, guessedLetters))
            print('------------')
            
            if getGuessedWord(secretWord, guessedLetters) == secretWord:
